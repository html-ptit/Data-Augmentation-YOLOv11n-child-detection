...
def adaptive_mask_value(orig_crop, child_crop):
    ...
    return int(np.clip(score * 255, 80, 200))  # Dynamic range control
def load_config(config_path='cfg_add_image.yaml'):
    """
...
class ImageAugmenter:
    ...
    def _place_object(self, 
        ...
        # Analyze background complexity using Laplacian variance
        gray_crop = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
        lap_var = cv2.Laplacian(gray_crop, cv2.CV_64F).var()
        
        # Compute dynamic alpha adjustment
        # Higher lap_var (complex background) → higher transparency reduction
        adjust_factor = np.clip(500 / (lap_var + 1e-5), 0.5, 1.5)
        alpha_dynamic = np.clip(alpha * adjust_factor, 0, 1)
        
        # Blend child onto original using adjusted alpha
    ...
    def augment_image(self, image_path: str, label_path: str) -> Tuple[str, str]:
        ...
        target_width, target_height = self._find_largest_bbox_of_class(bboxes, class_id=1, 
        ...
        for add_image_name in selected_add_images:
            ...
            zoom = self._calculate_zoom_factor(add_width, add_height, target_width, target_height)
            
            is_full = random.random() < self.full_ratio
            
            logger.debug(f"Adding {add_image_name} with zoom={zoom:.2f}, angle={angle:.1f}, is_full={is_full}")
